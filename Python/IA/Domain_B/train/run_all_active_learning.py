...
global_results = {} # Stockera tous les scores

# ==========================================
# 4. PHASE 1 : RANDOM ROBUST (La Référence)
# ==========================================
...
for r in range(N_ROUNDS_RANDOM):
    ...
    for step_idx, pct in enumerate(BUDGET_STEPS):
        ...
        model = ResNetForImageClassification.from_pretrained(model_A_path).to(device)
        model = train_standard(model, train_loader)
...
scores_mc = [rand_mean[0]] # Partage le même 0%
...
for pct in BUDGET_STEPS:
    ...
    print(f" -> {pct}% : {mse:.6f} (Modèle sauvegardé)")
...
def get_feats(model, files):
    model.eval()
    loader = DataLoader(ALDataset(files, processor), batch_size=64)
    feats = []
    with torch.no_grad():
        for pix, _ in loader: feats.append(model.resnet(pix.to(device)).pooler_output.squeeze().cpu().numpy())
...
for pct in BUDGET_STEPS:
    ...
    model = ResNetForImageClassification.from_pretrained(model_A_path).to(device)
    pool_f = get_feats(model, pool_out)
    known_f = get_feats(model, files_A + labeled_out)
...
print(f"✅ TOUT EST TERMINE ! Resultats dans : {output_dir}")