...
output_dir = script_dir / "AL_Results_Automated"
...
print("="*60)
print("⚙️ PHASE 1 : ENTRAÎNEMENT DES MODÈLES D'ACTIVE LEARNING")
print("="*60)
...
for pct in tqdm(BUDGET_STEPS, desc="Learning Loss Prog."):
    ...
    preds_loss = []
    with torch.no_grad():
        for pix, _ in tqdm(loader, desc=f"   ↳ Pred. {pct}%", leave=False):
    ...
    for epoch in range(AL_EPOCHS):
        ...
        for pix, tgt in loop:
            ...
            t_loss = torch.mean((preds - tgt.to(device))**2, dim=1)
...
print("\n✅ ENTRAÎNEMENTS TERMINÉS ! Les modèles sont sauvegardés.")