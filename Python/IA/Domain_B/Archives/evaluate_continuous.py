...
results_dir = script_dir / "AL_Results_Continuous"
...
print("="*60)
print("📊 PHASE 2 : ÉVALUATION DE L'ENTRAÎNEMENT CONTINU")
print("="*60)
...
def evaluate_mse(model_path, test_loader):
    ...
    try:
        ...
        print(f"Erreur lors de l'évaluation de {model_path}: {e}")
        return None
...
if not results_dir.exists():
    raise FileNotFoundError(f"Le dossier {results_dir} n'existe pas. Lancez d'abord l'entraînement !")

# --- 1. EVALUATION BASELINE (0%) ---
print("-> Calcul de la Baseline Initiale (Modèle A à 0%)...")
base_mse = evaluate_mse(model_A_path, test_loader)
global_results = {}

# --- 2. EVALUATION RANDOM ROBUST ---
print("-> Évaluation du Random Robust Continu...")
...
def plot_vs_random(strat_name, strat_scores):
    ...
    plt.title(f"{strat_name} vs Random (Apprentissage Continu)\nTest sur Séquences 9, 10, 11")
    plt.xlabel("% Domain B Annoté")
    plt.ylabel("MSE Loss")
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.legend()
    plt.xticks(BUDGET_WITH_ZERO)
    plt.savefig(results_dir / f"Compare_Continu_{strat_name.replace(' ', '_')}.png")
    plt.close()
...
plt.title("Comparaison Finale - Active Learning Continu\n(Test sur Séquences Temporelles Inconnues)")
plt.xlabel("% Domain B Annoté")
plt.ylabel("MSE Loss")
plt.grid(True, linestyle=':', alpha=0.6)
plt.legend()
plt.xticks(BUDGET_WITH_ZERO)
plt.savefig(results_dir / "GRAND_FINAL_CONTINUOUS.png")
